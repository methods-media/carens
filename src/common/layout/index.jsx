...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setShowScrollTop(scrolled > (totalHeight - viewportHeight) * 0.3);
    };
    ...
    <div>
      ...
      {pathname.includes('configurator') ? null :
        <div dir='ltr ' className={`fixed bottom-[1%] flex flex-col items-end gap-4 end-[1%] z-[50000] ${showScrollTop
          ...
          <button
            onClick={scrollToTop}
            className={` w-[50px] h-[35px] cursor-pointer  bg-black/60 text-white flex items-center justify-center rounded-lg shadow-[5px] transition-all duration-300 hover:bg-black `}
            style={{ zIndex: 1000 }}
          >
